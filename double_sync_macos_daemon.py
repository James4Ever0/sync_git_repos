...
def sweep():
    for cwd in getActiveVscodeWorkingDirs():
        if cwd in monitoringPaths:
            print("Vscode Running on {}".format(cwd))
            # change to that directory.
            os.chdir(cwd)
            # cwd = os.curdir
            # cwd = os.path.abspath(cwd)

            os.system("rm -rf {}/double_sync_macos.yml".format(scriptBase)) # anyway there should only be one such launcher here.
            # print(cwd)
            ...
            if (sessionName in getRunningTmuxSessionNames()) and (os.path.exists(lock_path)):
                ...
                # checkAttached()
                # exit()
                # do not exit please? do nothing excessive please?
            ...
            if sessionName in tmuxSessionNames:
                print("SKIP FOR PREVIOUS {} session to exit.".format(sessionName)) # this time we do not wait. we continue searching for hits.
                continue
                #     time.sleep(3)
                # else:
                #     break
            ...
            print("{} Not in monitoring paths".format(cwd))
